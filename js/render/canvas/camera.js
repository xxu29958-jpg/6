...
class Camera {
  ...
  setViewport(vw, vh) {
    ...
    if (aspect < 0.8) {
      ...
      /* 平板（V4.8 修复）：与桌面同一策略 min-fit 全图——横向不再裁掉
       * 世界左右两侧；焦点 +25 配重，顶对齐 */
      zoom = Math.min(vw / LAYERS.W, vh / LAYERS.H);
      fx = LAYERS.W / 2 + 25;
      fy = vh / zoom / 2;
      this.fgOnT = 1; this.vigT = 0.14;
    } else {
    ...
    /* 纵向余量保护：视图高度不超过世界+延展 */
    const maxVH = LAYERS.H + LAYERS.MT + LAYERS.MB;
  ...
  update(dt) {
    ...
    const zoomMin = Math.max(this.vw / (LAYERS.W + 120), this.vh / LAYERS.GH);
    if (this.zoom < zoomMin) this.zoom = zoomMin;
    const EI = 20;
    const halfW = this.vw / (2 * this.zoom), halfH = this.vh / (2 * this.zoom);
    const xLo = halfW - LAYERS.MX + EI, xHi = LAYERS.W + LAYERS.MX - EI - halfW;
    this.fx = xLo > xHi ? LAYERS.W / 2 : clamp(this.fx, xLo, xHi);
    const yLo = halfH - LAYERS.MT + EI, yHi = LAYERS.H + LAYERS.MB - EI - halfH;
    this.fy = yLo > yHi ? (LAYERS.H + LAYERS.MB - LAYERS.MT) / 2 : clamp(this.fy, yLo, yHi);
  }
...
/* Node 测试可加载（t8/t9 视口断言） */
const _camRoot = typeof window !== 'undefined' ? window : globalThis;
_camRoot.Camera = Camera;
