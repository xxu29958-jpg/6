...
/* compiled world 派生缓存（只读引用，非权威；init 后不变） */
let _streamM = null, _fallsR = null, _swingG = null;
function streamM() { if (!_streamM) _streamM = COMPILED.waterById('stream').measured; return _streamM; }
function streamLen() { return streamM().len; }
function fallsRects() {
  if (!_fallsR) _fallsR = [COMPILED.waterById('fallsUpper'), COMPILED.waterById('fallsMain')];
  return _fallsR;
}
function fallsPoolPt() { return COMPILED.waterById('fallsPool'); }
function pondDef() { return COMPILED.waterById('pond'); }
function swingGeo() {
  if (!_swingG) {
    const e = COMPILED.entities.swing, st = e.socketsWorld.stand;
    _swingG = { ax: e.transform.x, ay: e.transform.y, rope: e.props.rope,
      seatW: e.props.seatW, standX: st.x, standY: st.y };
  }
  return _swingG;
}
...
function initDynamic(mobile) {
  ...
  DYN.smokeT = COMPILED.chimneys.map(() => arand(0, 0.5));
  /* 漂浮微粒 */
  ...
  /* 水潭鱼（运行时粒子态；世界生成随机流末段，与 V4.7 调用序逐次一致） */
  DYN.fish.length = 0;
  for (let i = 0; i < 5; i++) {
    DYN.fish.push({
      a: rand(TAU),
...
function spawnLeaf() {
  ...
  if (!l) return;
  const z = COMPILED.entities.tree.zonesWorld.leafSpawn;
  l.x = arand(z.x0, z.x1);
...
function updateDynamic(dt) {
  ...
  for (const s of DYN.shimmer) {
    s.d += s.sp * dt;
    if (s.d > streamLen()) s.d -= streamLen();
  }
  ...
  /* 炊烟（夜晚减弱至一缕） */
  for (let ci = 0; ci < COMPILED.chimneys.length; ci++) {
    const ch = COMPILED.chimneys[ci];
    DYN.smokeT[ci] -= dt;
    if (DYN.smokeT[ci] <= 0) {
  ...
  if (DYN.rippleT <= 0) {
    ...
    if (rr) {
      const f = DYN.fish[Math.floor(arng() * DYN.fish.length)];
      rr.x = f.x + arand(-8, 8);
      rr.y = f.y + arand(-5, 5);
  ...
  /* 鱼（潭椭圆内巡游，点击受惊散开） */
  const fa = COMPILED.zones.fishArea;
  for (const f of DYN.fish) {
  ...
  if (!B.on) {
    ...
    B.x += B.vx * dt;
    if (B.x > COMPILED.bounds.w + 140) { B.on = false; B.next = arand(20, 50); }
  }

  /* 房屋窗光点击脉冲衰减（运行时态在 simulation 侧 RES.houseState） */
  for (const hid of Object.keys(RES.houseState)) RES.houseState[hid].flick *= Math.exp(-dt * 2.2);
}
...
function drawWater(g) {
  const c = DAY.cur, t = DYN.t, sm = streamM(), SH = XB.waterRender.streamClip;
  ...
  g.save();
  ribbonOrganic(g, sm, SH);
  g.clip();
  for (const s of DYN.shimmer) {
    polyAt(sm, s.d, _pt);
    const ox = -_pt.ty * s.off * SH * 1.4;
    const oy = _pt.tx * s.off * SH * 1.4;
...
function drawDriftLeaves(g) {
  const sm = streamM(), t = DYN.t, SH = XB.waterRender.streamClip;
  for (const l of DYN.driftLeaves) {
    const fade = smoothstep(0, 30, l.d) * (1 - smoothstep(sm.len - 60, sm.len - 12, l.d));
    if (fade < 0.03) continue;
    polyAt(sm, l.d, _pt);
    const ox = -_pt.ty * l.off * SH * 1.2;
    const oy = _pt.tx * l.off * SH * 1.2;
    g.save();
...
const SWING = {
  ...
  draw(g, t) {
    const s = swingGeo(), th = this.theta;
...
function drawCloudShadows(g) {
  ...
  for (let i = 0; i < XB.sky.clouds.length; i++) {
    const cl = XB.sky.clouds[i];
...
function drawGlows(g) {
  ...
  /* 星空（y<280 区域，twinkle） */
  if (c.starA > 0.02) {
    for (let i = 0; i < XB.stars.length; i++) {
      const s = XB.stars[i];
  ...
  if (night > 0.22) {
    ...
    const MN = XB.sky.moon;
    glow(MN.x, MN.y, 130, [200, 212, 228], 0.15 * ma);
    g.fillStyle = css([235, 230, 214], 0.9 * ma);
    g.beginPath(); g.arc(MN.x, MN.y, 24, 0, TAU); g.fill();
    g.fillStyle = css([188, 186, 170], 0.5 * ma);
    g.beginPath();
    g.arc(MN.x - 8, MN.y - 6, 5.5, 0, TAU);
    g.arc(MN.x + 9, MN.y + 8, 4, 0, TAU);
    g.fill();
  }
  ...
  if (gl >= 0.12) {
    /* 窗灯（逐个亮起 + 呼吸 + 点击脉冲）：窗/门世界坐标 = compiled 派生 socket */
    for (const h of COMPILED.entitiesByTag('house')) {
      const hst = RES.houseState[h.id];
      for (const wk of Object.keys(h.socketsWorld)) {
        if (wk.indexOf('win') !== 0) continue;
        const wn = h.socketsWorld[wk];
        let lit = smoothstep(wn.th, wn.th + 0.18, gl);
        if (lit <= 0.005) continue;
        lit *= 0.82 + 0.18 * Math.sin(t * 1.2 + wn.x * 0.7);
        lit *= 1 + hst.flick * 0.65 * Math.sin(t * 26);
        lit = clamp(lit, 0, 1);
        /* 窗芯：紧致柔光核 + 大晕（无硬边圆盘，融入插画墙面） */
        glow(wn.x, wn.y, 15, [255, 230, 184], lit * 0.85);
        glow(wn.x, wn.y, 58, warm, lit * 0.36);
      }
      /* 门口小暖光 */
      const dr = h.socketsWorld.door;
      const dlit = smoothstep(0.28, 0.55, gl) * 0.55;
      glow(dr.x, dr.y - 8, 30, warm, dlit * 0.3);
    }
    /* 灯笼：黄昏逐个亮、夜晚全亮带呼吸（灯头 = head socket；阈值在 binding） */
    for (const le of COMPILED.entitiesByTag('lantern')) {
      const ln = le.socketsWorld.head;
      const th = XB.lanternGlow[le.id];
      let lit = smoothstep(th, th + 0.16, gl);
      if (lit <= 0.005) continue;
  ...
  if (c.dusk > 0.2) {
    const d = c.dusk, sm = streamM(), SH = XB.waterRender.streamClip;
