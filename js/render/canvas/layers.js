...
/* V4.8：外扩边距/画幅为 renderer 常量（物理 bounds 由 setWorld 注入 compiled） */
const LAYERS = { ground: null, far: null, fg: null, vig: null, blob: null, disc: null,
  MX: 170, MT: 90, MB: 240, W: 2048, H: 1088, GW: 2388, GH: 1418 };
LAYERS.setWorld = function (bounds) {
  LAYERS.W = bounds.w; LAYERS.H = bounds.h;
  LAYERS.GW = bounds.w + LAYERS.MX * 2;
  LAYERS.GH = bounds.h + LAYERS.MT + LAYERS.MB;
};
...
function edgeFalloff(g, MX, W, skyY, deskY) {
  const GW = LAYERS.GW, GH = LAYERS.GH;
...
function bakeGround(terrain) {
  const W = LAYERS.W, H = LAYERS.H, MX = LAYERS.MX, MT = LAYERS.MT, MB = LAYERS.MB;
  const c = makeCanvas(LAYERS.GW, LAYERS.GH);
  const g = c.getContext('2d');

  /* 0) 整底 = 暗桌面色（任何缝隙都不露白） */
  g.fillStyle = '#221a12';
  g.fillRect(0, 0, LAYERS.GW, LAYERS.GH);
  ...
  g.save();
  g.translate(MX, MT);
  for (const a of XB.staticAssets) {
  ...
  const streamM = COMPILED.waterById('stream').measured;
  g.lineJoin = 'round'; g.lineCap = 'round';
  g.strokeStyle = 'rgba(26,22,15,0.10)';
  g.lineWidth = 12;
  ribbonOrganic(g, streamM, 50);
  g.stroke();
  g.strokeStyle = 'rgba(24,20,14,0.20)';
  g.lineWidth = 5;
  ribbonOrganic(g, streamM, 45);
  g.stroke();
  ...
  g.drawImage(c, 0, MT, LAYERS.GW, 2, 0, 0, LAYERS.GW, MT);
  ...
  const dg = g.createLinearGradient(0, MT + H - 30, 0, LAYERS.GH);
  ...
  g.fillStyle = dg;
  g.fillRect(0, MT + H - 30, LAYERS.GW, LAYERS.GH - (MT + H - 30));
  g.fillStyle = '#1c1510';
  g.fillRect(0, deskY, LAYERS.GW, LAYERS.GH - deskY);

  /* 5) 桌面质感：细密噪点 + 极轻横向木纹（确定性随机） */
  const trng = mulberry32(778899);
  for (let i = 0; i < 2600; i++) {
    const x = trng() * LAYERS.GW;
    const y = MT + H + 8 + trng() * (LAYERS.GH - MT - H - 8);
    const l = trng();
    g.fillStyle = l < 0.5
  ...
  for (let i = 0; i < 26; i++) {
    const y = MT + H + 20 + trng() * (LAYERS.GH - MT - H - 30);
    g.strokeStyle = 'rgba(0,0,0,' + (0.03 + trng() * 0.05) + ')';
    g.lineWidth = 0.8 + trng() * 1.4;
    g.beginPath();
    g.moveTo(0, y);
    g.bezierCurveTo(LAYERS.GW * 0.3, y + (trng() - 0.5) * 10,
      LAYERS.GW * 0.7, y + (trng() - 0.5) * 10, LAYERS.GW, y + (trng() - 0.5) * 6);
    g.stroke();
  }
  ...
  g.fillStyle = fg0; g.fillRect(0, 0, LAYERS.GW, MT);
...
function bakeFar(terrain) {
  const W = LAYERS.W, MX = LAYERS.MX, MT = LAYERS.MT;
  const bodyH = XB.ridgeMaxY + FAR_FEATHER;
  const FH = MT + bodyH + FAR_PAD;
  const c = makeCanvas(LAYERS.GW, FH);
  const g = c.getContext('2d');
  ...
  const MP = 32;
  const msk = makeCanvas(LAYERS.GW + MP * 2, FH + MP * 2);
  const mg = msk.getContext('2d');
  mg.fillStyle = '#fff';
  mg.fillRect(0, 0, LAYERS.GW + MP * 2, FH + MP * 2);
  mg.fillStyle = '#000';
  mg.beginPath();
  const rp = XB.ridgePts;
  mg.moveTo(0, MP + MT + rp[0][1]);
  /* 左右端用端点 y 水平延展到边带外缘（边带内容镜像翻折，脊线同理取平） */
  for (let i = 0; i < rp.length; i++) mg.lineTo(MP + MX + rp[i][0], MP + MT + rp[i][1]);
  mg.lineTo(LAYERS.GW + MP * 2, MP + MT + rp[rp.length - 1][1]);
  mg.lineTo(LAYERS.GW + MP * 2, FH + MP * 2);
  mg.lineTo(0, FH + MP * 2);
  mg.closePath();
  mg.fill();
  /* 羽化：蒙版整体 blur，使切割带 ≈ 脊线上下 30px 渐隐 */
  const msk2 = makeCanvas(LAYERS.GW + MP * 2, FH + MP * 2);
...
function drawScene(g) {
  ...
  const items = _scene;
  items.length = 0;
  for (const a of XB.dynAssets) { a._sy = a.ay; items.push(a); }
  /* 桥面绘制序基准 = deck surface 的 occlusion 元数据（map def 声明，renderer 消费） */
  const deckSortY = COMPILED.surfaceById('bridgeDeck').occlusion.sortY;
  for (const r of RES.list) {
    if (r.alpha <= 0.03) continue;
    if (r.state === 'swing') continue;          // 秋千上的居民由 SWING 层绘制
    r._sy = r._deck ? deckSortY + 1 : r.y;      // _deck = 当前 route segment 的 deck 成员推导
    items.push(r);
  }
...
/* ---------- 总入口（图片预加载完成后调用） ----------
 * 视觉水像素掩码已迁至 artcheck.js（仅作 art-vs-world evidence，物理查 compiled.waterAt） */
...
/* Node 测试可加载（LAYERS 常量 / 烘焙函数；相机 t8/t9 依赖 LAYERS 度量） */
const _layersRoot = typeof window !== 'undefined' ? window : globalThis;
_layersRoot.LAYERS = LAYERS;
_layersRoot.bakeAll = bakeAll;
_layersRoot.bakeScreenLayers = bakeScreenLayers;
_layersRoot.drawScene = drawScene;
_layersRoot.ribbonOrganic = ribbonOrganic;
_layersRoot.pondPath = pondPath;
if (typeof module !== 'undefined') module.exports = LAYERS;
