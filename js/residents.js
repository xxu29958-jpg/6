'use strict';
/* ============================================================
 * residents.js — 微小居民（12~14 个）状态机 + 手绘小人绘制
...
class Resident {
  constructor(i) {
    ...
    this.home = ['A', 'H', 'J'][i % 3];                 // 家节点（磨坊/右屋/左屋）
    const nk = Object.keys(WORLD.nodes);
  ...
  startWalk(dest, gohome) {
    ...
    const key = from + '>' + dest;
    let pts = WORLD.routes[key];
    if (!pts) {
      pts = WORLD.routes[dest + '>' + from];
      if (!pts) return false;
      pts = pts.slice().reverse();
    }
    this.route = pts; this.ri = 1;
    this.dest = dest; this.goingHome = !!gohome;
    /* Cb>Db = 桥面边：走桥面的居民画在桥之后（引道段正常 Y-sort，不再全程豁免） */
    this.onBridge = (from === 'Cb' && dest === 'Db') || (from === 'Db' && dest === 'Cb');
    this.carry = !gohome && arng() < 0.25;
    this.state = 'walk';
    return true;
  }

  /* V4.7 沿路网到目标节点（BFS 下一跳链），可选最后一段直线（须过水域断言） */
  routeTo(goal, final, gohome) {
    const near = WORLD.nearestNode(this.x, this.y);
    const nn = WORLD.nodes[near];
    this.hops = [];
    let cur = near, guard = 24;
    while (cur !== goal && guard-- > 0) {
      const nx = WORLD.nextHop[cur] && WORLD.nextHop[cur][goal];
      if (!nx) break;
      this.hops.push(nx); cur = nx;
    }
    this.final = final || null;
    this.goingHome = !!gohome;
    if (Math.hypot(nn.x - this.x, nn.y - this.y) > 6) {
      /* 先直线回最近节点（该段在干径上，过视觉水断言） */
      this.route = [[this.x, this.y], [nn.x, nn.y]];
      this.ri = 1; this.dest = near; this.onBridge = false; this.carry = false;
      this.state = 'walk';
  ...
  beginFinal() {
    ...
    this.ri = 1; this.dest = null; this.onBridge = false; this.carry = false;
    ...
    switch (this.state) {
      ...
      case 'inside':
        ...
        if (!RES.nightMode && night < 0.3) {
          this.exitDelay -= dt;
          if (this.exitDelay <= 0) {
            const dn = WORLD.nodes[this.home];
      ...
      case 'idle': {
        ...
        if (this.sitter && !RES.nightMode) {
          /* V4.7：坐姿者先沿路网走回桥西节点，再走最后一段直线上岸（废 onBridge 豁免） */
          const s = WORLD.sitSpot;
          this.routeTo('C', { x: s.x, y: s.y, state: 'sitprep' });
          break;
        }
        const opts = WORLD.adj[this.node];
      ...
      case 'sit': {
        this.sitT -= dt;
        if (RES.nightMode) { this.state = 'idle'; this.onBridge = false; this.timer = 0.1; break; }
        if (this.sitT <= 0) {
          this.sitT = arand(40, 90);
          this.state = 'idle'; this.onBridge = false; this.timer = arand(4, 10);
        }
        break;
      }
      ...
      case 'swing': {
        ...
        if (this.rideT <= 0 || RES.nightMode) {
          ...
          this.x = WORLD.swing.standX; this.y = WORLD.swing.standY;
          this.node = 'S';
  ...
  arrive() {
    ...
    this.dest = null;
    this.route = null; this.onBridge = false;
    /* V4.7 多跳路由：继续下一跳 / 转入最后一段直线 */
    ...
    if (this.finalState) {
      ...
      if (fs === 'sitprep') {
        ...
        this.state = 'sit'; this.sitT = arand(70, 130);
      } else if (fs === 'swingstart') {
        if (RES.nightMode || (SWING.rider && SWING.rider !== this)) { this.state = 'idle'; this.timer = arand(3, 8); }
        else { this.state = 'swing'; SWING.rider = this; this.rideT = arand(10, 16); this.facing = 1; }
...
function initResidents(mobile) {
  ...
  RES.list[0].sitter = true;
  RES.list[0].node = 'C';
  RES.list[0].x = 520; RES.list[0].y = 722; RES.list[0].timer = 2;
  RES.list[1].lantern = true;               // 夜晚提灯者
}
...
function updateResidents(dt) {
  ...
  if (!RES.nightMode && night > 0.55) {
    RES.nightMode = true;
    for (let i = 0; i < RES.list.length; i++) {
      const r = RES.list[i];
      if (i <= 3) {                          // 3~4 人留在室外
        if (r.sitter) { r.state = 'idle'; r.onBridge = false; r.timer = arand(20, 40); }
        continue;
      }
  ...
  } else if (RES.nightMode && night < 0.3) {
    RES.nightMode = false;
  }

  RES.chatCd -= dt;

  /* 秋千偶发乘坐（V4.7 频率×2：每 10~18s 挑一名空闲居民；夜晚不发起） */
  if (!RES.nightMode && !SWING.rider) {
    RES.swingT -= dt;
    if (RES.swingT <= 0) {
      RES.swingT = arand(10, 18);
      const cands = [];
      ...
      if (cands.length) {
        ...
        /* V4.7：沿路网走到秋千台节点 S，再走最后一段直线上台（过水域断言） */
        r.routeTo('S', { x: WORLD.swing.standX, y: WORLD.swing.standY, state: 'swingstart' });
      }
    }
  }
  ...
  for (const r of RES.list) {
    ...
    /* 开发期断言：居民脚底入水 → console.warn（节流 2s；过桥面/进出屋豁免）
     * V4.7：升级为「视觉水像素」——直接查 terrain 烘焙水掩码（溪流/水潭/瀑布池），
     * 与玩家肉眼所见水面严格一致，引道/沙岸不再误报 */
    if (!r.onBridge && r.alpha > 0.5 && WORLD.isWater &&
      (r.state === 'walk' || r.state === 'idle' || r.state === 'pause' ||
        r.state === 'sit' || r.state === 'chat' || r.state === 'noticed')) {
      if (WORLD.isWater(r.x, r.y + 1)) {
    ...
    for (let i = 0; i < RES.list.length; i++) {
      ...
      for (let j = i + 1; j < RES.list.length; j++) {
        ...
        if (d < 26 && arng() < 0.6) {
          ...
          a.chatT = b.chatT = arand(8, 15);
          a.partner = b; b.partner = a;
          a.facing = b.x >= a.x ? 1 : -1;
          b.facing = -a.facing;
          RES.chatCd = arand(22, 40);
          break outer;
        }
      }
    }
...
function drawPeeks(g) {
  ...
  for (const h of WORLD.houses) {
    ...
    const dx = h.door.wx, dy = h.door.wy;
    g.save();
    g.translate(dx, dy);
    g.globalAlpha = k;
    drawVillager(g, {
...
RES.peek = function (h) {
  if (DYN.t - h.peekT > 4) { h.peekT = DYN.t; h.flick = 1; }
};
